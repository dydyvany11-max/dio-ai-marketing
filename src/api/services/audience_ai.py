...
class GigaChatAudienceEnhancer:
    ...
    def extract_post_keywords_batch(self, texts: list[str]) -> dict[int, list[str]]:
        ...
        for start in range(0, len(prepared), POST_KEYWORD_BATCH_SIZE):
            ...
            prompt = (
                "Выдели для каждого Telegram-поста короткие ключевые фразы.\n"
                "Верни только один JSON-объект без markdown и пояснений.\n"
                "Формат ответа:\n"
                "{\n"
                '  "items": [\n'
                '    {"index": 0, "keywords": ["фраза 1", "фраза 2"]}\n'
                "  ]\n"
                "}\n"
                "Правила:\n"
                "- Для каждого поста верни от 2 до 5 коротких ключевых фраз.\n"
                "- Не придумывай темы вне текста.\n"
                "- Не добавляй общие слова, которые не раскрывают суть поста.\n\n"
            ...
            try:
                ...
                logger.warning("GigaChat post keyword extraction failed: %s", exc)
                continue
            ...
            for item in payload.get("items", []):
                ...
                if not isinstance(index, int) or not isinstance(keywords, list):
                    continue
                cleaned_keywords = [
                    self._truncate_text(str(keyword), 80)
                    for keyword in keywords
                    if str(keyword).strip()
                ]
                if cleaned_keywords:
                    result[index] = cleaned_keywords[:5]
        return result
    ...
    def enhance(self, report: TelegramAudienceReport) -> TelegramAudienceReport:
        ...
        try:
            ...
            snippet = self._safe_snippet(first_content)
            raise AIEnhancementError(
                f"GigaChat returned data in an invalid format. "
    ...
    def explain_competitor_match(
        ...
        prompt = (
            "Explain why one Telegram channel is similar to another.\n"
            "Write a short explanation in Russian in 1-2 sentences.\n"
            "No markdown, lists, disclaimers, or filler phrases.\n"
            "Be simple and business-friendly.\n"
            "If the channel is not a direct competitor, say what is missing.\n\n"
            f"Data:\n{json.dumps(payload, ensure_ascii=False)}"
        )
        content = self._chat_plain(prompt)
        text = self._clean_plain_response(content)
        if not text:
            raise AIEnhancementError("GigaChat did not return a plain competitor explanation")
        return text
    ...
    def extract_post_keywords_batch(self, texts: list[str]) -> dict[int, list[str]]:
        ...
        for start in range(0, len(prepared), POST_KEYWORD_BATCH_SIZE):
            batch = prepared[start : start + POST_KEYWORD_BATCH_SIZE]
            prompt = (
                "Extract short key phrases for each Telegram post.\n"
                "Return exactly one JSON object without markdown or explanations.\n"
                "Response format:\n"
                "{\n"
                '  "items": [\n'
                '    {"index": 0, "keywords": ["phrase 1", "phrase 2"]}\n'
                "  ]\n"
                "}\n"
                "Rules:\n"
                "- Return 2 to 5 short key phrases for each post.\n"
                "- Do not invent topics outside the text.\n"
                "- Avoid generic words that do not capture the post meaning.\n"
                "- Keywords themselves must be in Russian if the post is in Russian.\n\n"
                f"Posts:\n{json.dumps(batch, ensure_ascii=False)}"
            )
            try:
                content = self._chat(prompt)
    ...
    def _build_prompt(self, report: TelegramAudienceReport, *, compact: bool) -> str:
        payload = self._report_to_payload(report, compact=compact)
        return (
            "Build a Telegram channel audience analysis result.\n"
            "Return exactly one JSON object.\n"
            "No markdown, explanations, disclaimers, or any text before or after JSON.\n"
            "Do not use phrases like 'As a language model'.\n"
            "All text values in the JSON must be in Russian.\n"
            "If data is limited, stay cautious, but still return valid JSON.\n"
            "Infer audience only from posts, themes, and post metrics.\n"
            "Do not invent subscriber characteristics that are absent in the data.\n\n"
            ...
            [
                {
                    "role": "system",
                    "content": (
                        "Return JSON only. No explanations, no disclaimers, no markdown. "
                        "Any content outside JSON is forbidden. All text fields must be in Russian."
                    ),
                },
                {"role": "user", "content": prompt},
            ]
            ...
            [
                {
                    "role": "system",
                    "content": (
                        "Write briefly in Russian, without markdown, disclaimers, or filler. "
                        "Return only the useful plain-text answer."
                    ),
                },
                {"role": "user", "content": prompt},
            ]
    ...
    def _get_access_token(self) -> str:
        ...
        raw_key = (self._settings.authorization_key or "").strip()
        if not raw_key:
            raise AIEnhancementError("GIGACHAT_AUTH_KEY is not configured")
        ...
        token = data.get("access_token")
        if not token:
            raise AIEnhancementError(f"GigaChat OAuth did not return access_token: {data}")
    ...
    def _repair_response(self, bad_content: str, report: TelegramAudienceReport) -> str:
        payload = self._report_to_payload(report, compact=True)
        repair_prompt = (
            "Return only a valid JSON object for this audience analysis.\n"
            "No explanations, markdown, or disclaimers.\n"
            "Ignore the previous bad answer and rebuild the JSON from scratch.\n"
    ...
    def _clean_plain_response(content: str) -> str:
        ...
        for prefix in banned_prefixes:
            if text.startswith(prefix):
                raise AIEnhancementError("GigaChat returned boilerplate instead of the requested explanation")
        return text[:500]
    ...
    def _merge(report: TelegramAudienceReport, insights: AIAudienceInsights) -> TelegramAudienceReport:
        ...
        return replace(
            report,
            ai_enhanced=True,
            ai_message="GigaChat successfully enriched the audience profile from compressed channel signals.",
            dominant_theme=dominant_theme,
            channel_themes=channel_themes,
            audience_persona=audience_persona,
            summary=insights.summary,
